    ...
    if dataset_path.exists() and masks_dataset_path.exists():
        ...
        if len(masks_paths):
            ...
            for mask_path in masks_paths:
                ...
                print(
                    f"Processing: {mask_path}: {data_block.dtype} - Shape: {data_block.shape}"
                )
                ...
                for image_path in image_paths:
                    ...
                    if extracted_number == "729533":
                        ...
                        extracted_mask_block = np.flip(
                            np.transpose(extracted_mask_block, (-1, 1, 0)), axis=1
                        )
                        extracted_image_block = np.flip(
                            np.transpose(extracted_image_block, (-1, 1, 0)), axis=1
                        )

                    if extracted_image_block.ndim != 3:
                        raise ValueError(
                            f"Image has the following shape: {extracted_image_block.shape}"
                        )

                    if extracted_mask_block.ndim != 3:
                        raise ValueError(
                            f"Image has the following shape: {extracted_mask_block.shape}"
                        )

                    print(
                        f"Writing blocks from {extracted_number} to {images_output_path.parent}"
                    )
                    print(
                        f"Image shape: {extracted_image_block.shape} - Mask shape: {extracted_mask_block.shape}"
                    )
...
def percentile_normalization(data, percentiles=(1, 99), clip=True):
    ...
    if lower_val == upper_val:
        raise ValueError(
            "Percentile values are identical, division by zero would occur"
        )
...
def patch_image_mask_data(
    ...
    padding_needed = [
        (patch_size - (dim % patch_size)) % patch_size for dim in data_block.shape
    ]
    ...
    if apply_percentile_norm:
        ...
        padded_data_block = percentile_normalization(
            data=padded_data_block, percentiles=(pmin, pmax), clip=False
        )
    ...
    for i, patched_mask_block in enumerate(patched_mask):
        ...
        saved_blocks += 1
        output_image_block = output_images.joinpath(
            f"{smartspim_id}_image_block_{i}.tif"
        )
    ...
    print(
        f"Total blocks: {n_blocks} - Saved blocks: {saved_blocks} - Empty blocks: {n_blocks - saved_blocks}"
    )
...
def process_slice(
    ...
    if max_id and max_data_block:
        ...
        output_image_slice = output_images.joinpath(
            f"{smartspim_id}_image_slice_{i}_ax_{axis}_{channel_name}.tif"
        )
        output_mask_slice = output_masks.joinpath(
            f"{smartspim_id}_mask_slice_{i}_ax_{axis}_{channel_name}.tif"
        )
...
def patch_image_in_slices(
    ...
    if apply_percentile_norm:
        print("Applying percentile norm")
        data_block = percentile_normalization(
...
if __name__ == "__main__":
    ...
    for patch_size in patch_sizes:
        for step_size in step_sizes:
            print(
                f"Processing with patch size of {patch_size} and step size of {step_size} - Percentiles: {apply_percentile_norm} - clip int {clip_int}"
            )
